...
function Uniform_Split() {
  ...
  const handleAddExpense = async (event) => {
    ...
    try {
      ...
      const createExpenseResponse = await axios.post('http://localhost:5000/expenses', expenseDataWithId);
      const group_expenseData = {
        expense_id: newExpenseId,
        group_id: groupId
      };
      const createGroup_ExpenseResponse = await axios.post('http://localhost:5000/group_expenses', group_expenseData);
  ...
  const handleDone = async (event) => {
    ...
    try {
      ...
      await Promise.all(selectedParticipants.map(async (participantId) => {
        ...
        const createExpenseResponse = await axios.post('http://localhost:5000/split', expenseData);
