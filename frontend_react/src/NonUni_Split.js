...
function NonUni_Split() {
    ...
    const fetchData = async () => {
      try {
        const [participantsRes, usersRes] = await Promise.all([
          axios.get("http://localhost:5000/members"), // Fetch members
          axios.get("http://localhost:5000/user"), // Fetch users
        ]);
        ...
        const matchedMembers = membersData.filter(
          (member) => member.group_id === groupId && member.member_id !== userId
        );
  ...
  const handleCheckboxChange = (event) => {
    ...
    if (checked) {
      ...
      setSelectedParticipants(
        selectedParticipants.filter((participant) => participant !== value)
      );
    }
  };
  ...
  const handleAddExpense = async (event) => {
    ...
    try {
      ...
      const maxExpenseId = Math.max(
        ...expenses.map((expense) => parseInt(expense.expense_id))
      );
      ...
      const createGroup_ExpenseResponse = await axios.post(
      ...
      if (createExpenseResponse.status === 201) {
        ...
        console.error(
          ...
          createExpenseResponse.statusText
        );
      }
    } catch (error) {
    ...
    try {
      ...
      await Promise.all(
        selectedParticipants.map(async (participantId) => {
          ...
          const sharedExpense = participantShares[participantId]
            ? parseFloat(participantShares[participantId])
            : 0;
          const expenseData = {
  ...
  return (
    <div className="NonUni_Split">
      ...
      <p>
        {" "}
        <span className="page-head-1">Add an Expense (Non-Uniform)</span>
      </p>
      <br />
      <br />
        ...
        <form onSubmit={handleAddExpense}>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setInputData({ ...inputData, date_time: e.target.value })
              }
            />
            <br />
            <br />
      ...
      <p>
        {" "}
        <span className="normal-info">Choose participants for expense</span>
      </p>
          ...
          {expenseInfo.map((participant) => (
            <div key={participant.user_id}>
              ...
              <input
                ...
                onChange={(e) =>
                  handleParticipantShareChange(e, participant.user_id)
                }
